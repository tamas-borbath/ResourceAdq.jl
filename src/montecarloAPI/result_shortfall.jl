...
mutable struct MCAShortfallAccumulator <: ResultAccumulator{MonteCarloAPI,Shortfall}
...
function merge!(
    x::MCAShortfallAccumulator, y::MCAShortfallAccumulator
)
...
accumulatortype(::MonteCarloAPI, ::Shortfall) = MCAShortfallAccumulator

function accumulator(
    sys::SystemModel{N}, ::MonteCarloAPI, ::Shortfall
) where {N}
    ...
    return MCAShortfallAccumulator(
...
function reset!(acc::MCAShortfallAccumulator, sampleid::Int)
...
struct MCAShortfallSamplesAccumulator <:
    ResultAccumulator{MonteCarloAPI,ShortfallSamples}
...
function merge!(
    x::MCAShortfallSamplesAccumulator, y::MCAShortfallSamplesAccumulator
)
...
accumulatortype(::MonteCarloAPI, ::ShortfallSamples) = MCAShortfallSamplesAccumulator

function accumulator(
    sys::SystemModel{N}, simspec::MonteCarloAPI, ::ShortfallSamples
) where {N}

    nregions = length(sys.regions)
    shortfall = zeros(Int, nregions, N, simspec.nsamples)

    return MCAShortfallSamplesAccumulator(shortfall)

end

function record!(
    acc::MCAShortfallSamplesAccumulator,
    system::SystemModel{N,L,T,P,E},
    state::SystemState, problem::DispatchProblem,
    sampleid::Int, t::Int
) where {N,L,T,P,E}
...
reset!(acc::MCAShortfallSamplesAccumulator, sampleid::Int) = nothing
