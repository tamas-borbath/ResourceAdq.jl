...
struct MCAUtilizationAccumulator <: ResultAccumulator{MonteCarloAPI,Utilization}
...
function merge!(
    x::MCAUtilizationAccumulator, y::MCAUtilizationAccumulator
)
...
accumulatortype(::MonteCarloAPI, ::Utilization) = MCAUtilizationAccumulator

function accumulator(
    sys::SystemModel{N}, ::MonteCarloAPI, ::Utilization
) where {N}
    ...
    return MCAUtilizationAccumulator(
        util_interface, util_interfaceperiod,  util_interface_currentsim)

end

function record!(
    acc::MCAUtilizationAccumulator,
...
function reset!(acc::MCAUtilizationAccumulator, sampleid::Int)
...
struct MCAUtilizationSamplesAccumulator <:
    ResultAccumulator{MonteCarloAPI,UtilizationSamples}
...
function merge!(
    x::MCAUtilizationSamplesAccumulator, y::MCAUtilizationSamplesAccumulator
)
...
accumulatortype(::MonteCarloAPI, ::UtilizationSamples) = MCAUtilizationSamplesAccumulator

function accumulator(
    sys::SystemModel{N}, simspec::MonteCarloAPI, ::UtilizationSamples
) where {N}

    ninterfaces = length(sys.interfaces)
    utilization = zeros(Float64, ninterfaces, N, simspec.nsamples)

    return MCAUtilizationSamplesAccumulator(utilization)

end

function record!(
    acc::MCAUtilizationSamplesAccumulator,
    system::SystemModel{N,L,T,P,E},
    state::SystemState, problem::DispatchProblem,
    sampleid::Int, t::Int
) where {N,L,T,P,E}
...
reset!(acc::MCAUtilizationSamplesAccumulator, sampleid::Int) = nothing
